...
import html
import unicodedata
# IMPORT DARI SERVICE SEBELAH (Relative Import)
from Services.kalimat_break_service import KalimatBreakService
...
class PreprocessingService:
    ...
    def removeHTMLString(data: str) -> str:
        ...
        # Aturan custom
        data = PreprocessingService._fix_em_dot_rule(data)
    ...
    def NormalizeDataInput(payload):
        ...
        for key, label in data_to_process:
            ...
            cleaned = PreprocessingService.removeHTMLString(raw_value)
    ...
    def preprocess_and_split_proposal(payload):
        ...
        daftarParagraf = PreprocessingService.NormalizeDataInput(payload)

        # 2) Pecah jadi kalimat dengan KalimatBreak
        #    diasumsikan breakKalimat mengembalikan:
        #    [ [idProposal, isiKalimat, aspek], ... ]
        daftarKalimatProposal = KalimatBreakService.breakKalimat(daftarParagraf)

        # 3) Susun output final untuk API
        result = []
        for idx, (pid, txt, asp) in enumerate(daftarKalimatProposal, start=1):
            txt_bersih = PreprocessingService._fix_name_typos_final(PreprocessingService._normalize_ws_final(txt))
            result.append(
                {
                    "sentence_id": idx,
                    "proposal_id": str(pid),
